...
void nn_xor(Tensor_t<T> input, Tensor_t<T> labels, int iters)
{
    ...
    Optimizer<T> Op(params, 0.07, ADAM, true);
    ...
    for(int iter = 1; iter <= iters; iter++)
    {
        ...
        Tensor_t<T> b = a->sigmoid();

        Tensor_t<T> c = l2.forward(b);
        Tensor_t<T> out = c->sigmoid();
...
class Model{
    ...
    public:
            ...
            l3(10, 64, true)
        {
            ...
            parameters.insert(parameters.end(), param2.begin(), param2.end());
            std::vector<Tensor_t<T>> param3 = l3.parameters();
            parameters.insert(parameters.end(), param3.begin(), param3.end());
        }
        ...
        void train(int epoch, int iters, Tensor_t<T> x, Tensor_t<T> y) {  
            Optimizer<T> Op(parameters, 0.1, ADAM, true);
            std::cerr << "Train.....................................................................\n";
            for (int iter = 0; iter < (iters*epoch); iter++) {
                Op.zero_grad();              // must zero grads each iteration

                Tensor_t<T> ypred = this->forward(x);            // run forward pass first
        ...
        void load(std::string path) {
            ...
            auto all = this->parameters;  // get all parameters recursively
...
void nn_mnist()
{
    Matrix<T> raw_train = Tabular<T>(true).load("Datasets/MNIST_CSV/mnist_train.csv");
    ...
    Tensor_t<T>  X = make_tensor<T>(x_train);
    Tensor_t<T>  y = make_tensor<T>(Matrix<T>::one_hot(labels_train, 10));
    auto epoch = 10, iters = 10;

    // FeedForward<T> model(x_train.shape[1], 64, 10);
    Model<T> model(x_train.shape[1]);
    model.load("Models/model.hge");

    Optimizer<T> Op(model.parameters, 1e-3, ADAM, true);
    std::cerr << "Train.....................................................................\n";
    ...
    for (int iter = 0; iter < (iters*epoch); iter++) 
    {
        ...
        {   // ← inner scope
            ...
            Tensor_t<T> loss = Tensor<T>::mse(y, ypred);
            loss->backward(Matrix<T>({1}));
            if (iter % iters == 0) {
                std::cerr << "iter: " << iter << " loss %: " << loss->val * 100 << "\n";
            }
    ...
    Matrix<T> raw_test = Tabular<T>(true).load("Datasets/MNIST_CSV/mnist_test.csv");
...
int main()
{
    ...
    cout << "\n=== Serialize test ===\n";
    Matrix<float> raw_test = Tabular<float>(true).load("Datasets/MNIST_CSV/mnist_test.csv");
