...
#include "tools/Profiler/HyperparamAdvisor.hpp"   // for QuantPolicy in workload lambdas
#include "net/Protocol/MessageProtocol.hpp"
#include "net/Protocol/MessageTransport.hpp"
...
int main(int argc, char* argv[]) {
    ...
    GPT<float, Linear>* model_ptr = nullptr;  // heap so we control lifetime  

    profiler.profile_load([&]() {
        GPTGGUFLoader<float, Linear> loader;
        model_ptr = new GPT<float, Linear>(std::move(loader.load_model(model_path, hp)));
