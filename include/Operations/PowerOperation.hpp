...
*/
    ...
    {
        this->t1->backward(grad * this->t2->val * pow(this->t1->val, (this->t2->val - Matrix<T>(1))));
    }

    template <typename T>
    Tensor_t<T> PowerOperation<T>::forward()
    {
        return std::make_shared<Tensor<T>>(pow(this->t1->val, this->t2->val), this->shared_from_this());
    }
