...
class LlamaGGUFLoader : public GGUFLoader<T> {
    ...
    void load_attention_heads(MultiHeadAttention<T, LinearT>& mha,
        ...
        for (size_t h = 0; h < n_heads; h++) {
            Head<T, LinearT>& head = *mha.Heads[h];
    ...
    void load_block(Llama<T, LinearT>& model, const LlamaHyperParams& hp, size_t blk_idx) {
        Block<T, LinearT>&   block = *model.decoder_blocks[blk_idx];
    ...
    void load_weights(Llama<T, LinearT>& model, const LlamaHyperParams& hp) {
...
public:
    Llama<T, LinearT> load_model(const std::string& path, const LlamaHyperParams& hp) {
        this->open(path);
        std::cout << "[LlamaGGUFLoader] Building Llama skeleton"
                  ...
                  << " heads="  << hp.n_heads
                  << " kv_h="   << hp.n_kv_heads << "\n";
        Llama<T, LinearT> model(hp.vocab_size, hp.input_dim, hp.block_size,
