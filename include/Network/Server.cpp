 ...
 *    • ALL aggregation math lives in Trainer:
 *        trainer.aggregate(updates)          → FedAvg (was federatedAverage+applyUpdate)
 *        trainer.aggregate_logits(logits)    → FedDistill consensus
 *        trainer.get_flat_weights()          → serialise for broadcast
 *    • Server is pure networking + round coordination:
 *        handleClient()         → receive delta bytes → reconstruct tensors → push to updates[]
 *        handleClientLogits()   → receive logit bytes → push to logit_updates[]
 *        broadcast()            → trainer.get_flat_weights() → write to sockets
 *        broadcastLogits()      → write consensus to sockets
...
// (Trainer<GPT<T>,T>, lr=0) owns all aggregation math. FederatedServer calls:
//   trainer.aggregate(updates)        — FedAvg
//   trainer.aggregate_logits(logits)  — FedDistill consensus
//   trainer.get_flat_weights()        — already called inside broadcast()
#include "Server.hpp"
...
//    • broadcast()                      calls trainer.get_flat_weights() internally
...
class FederatedServer : public Server<T> {
    ...
    // ── FedAvg round ──────────────────────────────────────────────────────────
    //
    //  1. Wait until min_clients updates arrive (handleClient notifies).
    //  2. trainer.aggregate(updates) — FedAvg math in Trainer, not here.
    //  3. broadcast() — trainer.get_flat_weights() → write to sockets.

    void run_fedavg_round() {
        size_t round_no = ++total_rounds;
        Timer  round_t;

        // Wait for enough client updates. handleClient() (base class) notifies.
        size_t n_clients;
        {
        ...
        // ── Aggregate: all FedAvg math lives in Trainer ───────────────────────
        Timer agg_t;
        {
            std::lock_guard<std::mutex> lk(locker);
            // trainer.aggregate() = federatedAverage() + applyUpdate() combined.
            // It averages client delta tensors and writes the result back into
            // model.parameters() (the global model) in one call.
            trainer.aggregate(updates);
            updates.clear();
        }
        ...
        // ── Broadcast: Server.broadcast() uses trainer.get_flat_weights() ─────
        // No aggregation logic here — just networking.
