...
//  * blk.N.attn_q.weight         [nH*hs, D]           {nH*hs, D}        slice_rows → {hs, D} ✓
//  * blk.N.attn_k.weight         [nKV*hs, D]          {nKV*hs, D}       slice_rows → {hs, D} ✓
//  * blk.N.attn_v.weight         [nKV*hs, D]          {nKV*hs, D}       slice_rows → {hs, D} ✓
...
//         // slice_rows extracts {hs, D} = {out, in} per head — no transpose needed.
...
//                     this->slice_rows(Q_full, h * head_size, (h + 1) * head_size),
//                     pfx + "attn_q[h=" + std::to_string(h) + "]");

//             if (K_full.get_size() > 0)
//                 this->copy_into(head.K.weight,
//                     this->slice_rows(K_full, kv_h * head_size, (kv_h + 1) * head_size),
//                     pfx + "attn_k[kv=" + std::to_string(kv_h) + "]");

//             if (V_full.get_size() > 0)
//                 this->copy_into(head.V.weight,
//                     this->slice_rows(V_full, kv_h * head_size, (kv_h + 1) * head_size),
...
//     void load_block(Llama<T>& model, const LlamaHyperParams& hp, size_t blk_idx) {
